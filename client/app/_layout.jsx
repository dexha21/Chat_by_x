...
function RootLayoutInner() {
  ...
  const { isReady, downloadMessages, getChatLastUpdatedAt, InsertChats, InsertStories, getStoriesLastUpdatedAt, downloadStories, cleanLocal_ } = useChatDb();
  ...
  useEffect(() => {
    ...
    const connect = async() => {
      ...
      const url = `http://192.168.0.100/Chat_by_x/server/public/api/live-chats?token=${encodeURIComponent(token)}&last_updated_at=${encodeURIComponent(lastChatAtRef.current)}`;

      console.log("Chat URL", url);
      
      const es = Platform.OS === 'web' ? new window.EventSource(url) : new EventSource(url);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      const url = `http://192.168.0.100/Chat_by_x/server/public/api/live-stories?token=${encodeURIComponent(token)}&last_updated_at=${encodeURIComponent(lastStoryAtRef.current)}`;
